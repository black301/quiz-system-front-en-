...
export function GradeSubmissionPage({ submissionId, onBack }: GradeSubmissionPageProps) {
  ...
  const fetchSubmissionData = async () => {
    try {
      ...
      const submissionData = await apiFetch(`/instructor/submissions/${submissionId}/`)
      setSubmission(submissionData)

      // Fetch quiz questions to get full question details
      const quizData = await apiFetch(`/instructor/quizzes/${submissionData.quiz}/`)
  ...
  const handleGradeAllAnswers = async (
    gradedAnswers: Array<{ answer_id: number; points: number; feedback: string }>,
  ) => {
    try {
      setIsSaving(true)
      setError(null)
  ...
  const handleEditSingleAnswer = async (answerId: number, points: number, feedback: string) => {
    try {
      setError(null)
  ...
  const calculateTotalGrade = () => {
    ...
    return submission.answers.reduce((total, answer) => total + (answer.points || 0), 0)
  }
    ...
    return (
      <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-body-color dark:text-dark-6">Loading submission...</span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-dark dark:text-white mb-2">Error Loading Submission</h3>
  ...
  const percentage = totalPossible > 0 ? Math.round((totalGrade / totalPossible) * 100) : 0
      ...
      <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-body-color dark:text-dark-6">Submitted on {formatDate(submission.submission_date)}</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
            ...
            <p className="text-lg font-semibold text-dark dark:text-white">{submission.student_name}</p>
            <p className="text-sm text-body-color dark:text-dark-6">ID: {submission.student}</p>
          </div>
          <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
            <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">Submission Date</h4>
            <p className="text-lg font-semibold text-dark dark:text-white">{formatDate(submission.submission_date)}</p>
          </div>
          <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
            <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">Questions</h4>
            <p className="text-lg font-semibold text-dark dark:text-white">{submission.answers.length}</p>
          </div>
        </div>
      </div>
